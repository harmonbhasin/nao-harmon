...
# Added this to see which samples are the issue, I have an idea.
basic_stats_raw_metrics <- basic_stats_raw_metrics %>%
  mutate(harmon_value = case_when(
    metric == "# Read pairs" & value > 4e+07 ~ 4e+07,
    metric == "Total base pairs\n(approx)" & value > 1.25e+10 ~ 1.25e+10,
    TRUE ~ value
  ))
# Raw
...
# Changed
g_basic <- ggplot(basic_stats_raw_metrics, 
                  aes(x=date, y=harmon_value, fill=na_type)) +
  geom_col(position = "dodge") +
  scale_x_discrete(name="Collection Date") +
  scale_y_continuous(expand=c(0,0)) +
  expand_limits(y=c(0,100)) +
