...
subtitle: "Air filters from a daycare in Virginia."
...
# Data input paths
data_dir <- "./"
libraries_path <- file.path(data_dir, "harmon-sample-metadata.csv")
...
g_basic <- ggplot(basic_stats_raw_metrics, 
  ...
  #facet_grid(metric~ctrl, scales = "free", space="free_x", switch="y", ylim = list(
  #  "# Read pairs" = c(0, 4e+07),
  #  "% Duplicates\n(FASTQC)" = c(0, 1.25e+10)
  #)) +
  theme_rotate + theme(
...
kraken_reports <- read_tsv(sprintf('%s/kraken_reports.tsv', data_dir))
...
hv_reads_filtered <- read_tsv(sprintf('%s/hv_hits_putative_collapsed.tsv', data_dir))
hv_reads_filtered <- left_join(hv_reads_filtered, libraries)
...
Harmon note: I did not run BLAST
...
Harmon note: picking up back here.
...
mrg_hv_named <- mrg_hv %>% mutate(taxid = as.integer(taxid)) %>% left_join(viral_taxa, by="taxid")
